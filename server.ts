...
async function startServer() {
  ...
  const getSessionUser = (req: express.Request) => {
    ...
    try {
      ...
      // For local demo, we can just base64 decode the session object
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      // Generate a simple token (base64 of session) for demo/client transport
  ...
  // Auth: Register (Create tenant + owner member)
  ...
  // Services: Get public active services
  ...
  // Appointments: Admin Get List with Filters
  ...
  // Appointments: Admin Status Update
  app.put('/api/appointments/:id/status', requireAuth, async (req, res) => {
    try {
      ...
      // Read appointments to verify ownership
  ...
  // Appointments: Generate Available Time Slots (Public scheduling page)
  app.get('/api/appointments/slots', async (req, res) => {
    try {
      ...
      const opening = tenant.opening_time.substring(0, 5); // "HH:MM"
      const closing = tenant.closing_time.substring(0, 5); // "HH:MM"
      const duration = service.duration_minutes;

      // Get all appointments on that date to compute overlap
      ...
      // Create 30-min intervals and test feasibility
      while (current < closing) {
        ...
        // Advance slot pointer by 30 mins
  ...
  // Appointments: Create (Public booking route with validation)
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`LavaAgenda server running on http://localhost:${PORT}`);
  });
}
